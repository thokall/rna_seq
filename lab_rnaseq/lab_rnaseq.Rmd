...
There are many public reposotories that holds RNA-sequence data and for some of them one can access not only the raw read data, but also the .bam files or even the count matrix. This is useful as mapping the reads is a time consuming, but not very difficul step. Since we within the course do not have accesss to any compute resources besides our laptops, we will not focus our efforts on read mapping. All teachers involved have experiences in mapping rna-seq data to genomes so do not hesitate in asking questions relatede to this

# Access public RNA-seq data
RNA-seq data are deposited in most cases made public via open sequence repositories such as [https://www.ncbi.nlm.nih.gov/geo/](NCBIs Gene Expression Omnibus (GEO)) and [https://www.ebi.ac.uk/arrayexpress/](EBIs ArrayExpress). Both of these can be queried by just visiting the webpages, but they also have bioconductor packages so that one can query and download data directly from R. 
